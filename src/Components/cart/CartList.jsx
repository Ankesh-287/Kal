            ...
            {cartItems.map((item, index) => (
                <Grid container key={`${item.id}-${item.color}-${item.size}-${index}`}
                    ...
                    <Grid item xs={2} sm={2} sx={{ display: 'flex', alignItems: 'center' }}>
                        ...
                        <Typography sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', borderTop: '1px solid grey', borderBottom: '1px solid grey', width: 40, height: 41.5 }}>{item.quantity}</Typography>
